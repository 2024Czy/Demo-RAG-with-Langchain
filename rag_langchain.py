...
'''
待改进的点：
1、实现记忆，多轮对话。添加问题和回答存储，超过存储空间做一个部分清除
2、根据预期回答做一个联网判断回滚
3、去噪优化，对于ppt 类的 pdf 的数据个性化去噪，很多页只有一个名词；对html 着重处理数据去噪
4、做一个是否采纳回答的功能。如果用户采纳则讲问答和来源放入新建知识库，对这些常见问题使用常见知识库进行快速检索回答。
这需要做一个库内相似度得分判断，同时还要能根据来源检索特定分块，对来源准确性要求很高，溯源工作要使用id再次优化。
...
if __name__ == '__main__':
    SILICONFLOW_API_KEY = os.environ.get("SILICONFLOW_API_KEY") # SiliconFlow API 密钥
    SERPAPI_KEY = os.environ.get("SERPAPI_KEY") # SerpApi API 密钥
    data_directory = os.path.join(os.path.dirname(__file__), "data") # 初始数据文件夹路径
    append_files_path = os.path.join(os.path.dirname(__file__), "append_data") # 追加数据文件夹路径
    embedding_model_name = "Qwen/Qwen3-Embedding-0.6B" # 词向量模型名称
    collection_name = "test-clean" # 数据库名称
    persist_directory = os.path.join(os.path.dirname(__file__), "chroma3-clean") # 数据库持久化存储文件夹路径
    top_relevant_k= 10 # 取前top_relevant_k个相关文档
    local_model_name = "google/gemma-2-2b-it" # 本地模型名称
    ...
    if db is None:
        docs = load_directory(data_directory) # 加载文件夹数据
        chunks = split_text(docs, chunk_size=200, chunk_overlap=30) # 数据切分
        db = store_chroma( embedding_model_name,collection_name, persist_directory,documents=chunks) # 存储到数据库
        db = load_chroma(embedding_model_name, persist_directory, collection_name) # 重新加载数据库
    # 追加文件夹数据（必要时启用）
    # db = store_chroma(embedding_model_name,collection_name, persist_directory, db=db, append_files_path= append_files_path)
    retriever = build_retriever(db, search_type="similarity", search_kwargs={"k": top_relevant_k}) # 构建检索器
    # 获取检索结果
